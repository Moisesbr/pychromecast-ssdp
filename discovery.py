...
def discover_chromecasts(max_devices=None, timeout=DISCOVER_TIMEOUT):
    ...
    client = SSDPClient(timeout=5)
    dev_list = []
    devices = client.m_search(
        st="ssdp:urn:dial-multiscreen-org:service:dial:1")
    for device in devices:
        try:
            f = urllib.request.urlopen(device['location'])
            response = f.read().decode('utf-8')
            root = ET.fromstring(response)
            ip_address = device['location'].split(":")
            ip_address = ip_address[1].replace("/", "")
            friendly_name = root[2][1].text
            manufacturer = root[2][2].text
            modelName = root[2][3].text
            uuid = UUID(root[2][4].text)
            device_data = (ip_address, 8009, uuid, 'Chromecast', friendly_name)
            dev_list.append(device_data)
            print(dev_list)
